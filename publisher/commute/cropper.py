...
class DocumentAdjustments:
    ...
    def read_pdf_size(document_name, format_parameters):
        ...
        total_height: int = 0
        width: int = 0
        total_height, width = DocumentAdjustments.result_width_and_height(
            format_parameters, number_of_pages, read_pdf, total_height, width,
            write_pdf
        )

        result = DocumentAdjustments.cut_out_pdf(document_name, total_height, width, write_pdf)
        return result
    ...
    def make_pdf_preview(document_name):
        try:
            ...
            for page in pdf_image:
                ...
                page.save(f"{updated_path}_crp.jpeg", 'JPEG')
    ...
    def auto_crop_pdf(format_parameters):
        ...
        try:
            ...
            pdf_cropped: str = f'{document_name}_crp'
            DocumentAdjustments.make_pdf_preview(document_name)
            return DocumentAdjustments.read_pdf_size(pdf_cropped, format_parameters)
        except FileNotFoundError:
            logging.info(fstring.message["exception"]['file_not_found'])
        except ValueError:
            logging.info(fstring.message["exception"]['value'])
