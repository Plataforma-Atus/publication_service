...
def extract_text(format_parameters):
    ...
    for idx, format_parameters['paragraph'] in enumerate(format_parameters['document'].paragraphs):
        if index > 0:
            ...
            if format_parameters['paragraph'].text != '':
                TextReading.run_text(format_parameters)


class Insert:

    @staticmethod
    def date(format_parameters: dict) -> None:
        days: date = format_parameters['days']
...
class TextReading:
    ...
    @staticmethod
    def run_text(format_parameters: dict):
        if format_parameters['bold'] or format_parameters['italic'] or format_parameters['underline']:
            if len(format_parameters['texts_flowing'].runs) == 0:
                ...
                Insert.set_color(format_parameters)
    ...
    @staticmethod
    def read_text(format_parameters: dict):
...
class Format:
    ...
    def format_margins(format_parameters: dict):
        document_margins: list[int] = [0, 0, 0, 0]
    ...
    def format_page(format_parameters: dict):
        for i in range(len(format_parameters['docx_default'].paragraphs)):
            ...
            Insert.add_edge(format_parameters, i)
    ...
    @staticmethod
    def special_adjust(format_parameters: dict, column=21):
...
def save_document(format_parameters: dict, chars_count=0):
...
class Standardizer:
    ...
    def docx(format_parameters: dict):
        ...
        TextReading.read_text(format_parameters)
        extract_text(format_parameters)
        find_replace(format_parameters)
        Insert.date(format_parameters)
        Format.format_page(format_parameters)
        Format.special_adjust(format_parameters)
